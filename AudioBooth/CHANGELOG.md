...
### Added
- Player lock mode - Lock the player controls to prevent accidental taps during playback
- Series filtering - Filter the series list on the series page
...
### Changed
- xHE-AAC support - Improved playback support for xHE-AAC (USAC) encoded audio files
- Library filters - Filters are now cached for faster loading and better app performance
- Unsync session logic - Improved unsynced session handling to discard sessions that were already uploaded
- Smart continue - Improved smart continue when moving to the next episode, book, or item
